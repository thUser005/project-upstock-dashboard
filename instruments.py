...
def get_done_marker():
    return os.path.join(get_today_dir(), ".done")

def is_bootstrap_done_today():
    return os.path.exists(get_done_marker())

def mark_bootstrap_done():
    os.makedirs(get_today_dir(), exist_ok=True)
    with open(get_done_marker(), "w") as f:
        f.write("ok")
...
# ===========================
# Bootstrap (Safe Single Run)
# ===========================

def bootstrap_instruments(overwrite=False):
    global BOOTSTRAP_DONE

    # Disk-level guard (multi-process safe)
    if is_bootstrap_done_today():
        log("Instrument bootstrap already completed today — skipping")
        BOOTSTRAP_DONE = True
        return

    # Memory-level guard (multi-thread safe)
    if BOOTSTRAP_DONE:
        return
